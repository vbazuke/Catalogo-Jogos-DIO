...
namespace ExemploApiCatalogoJogos.Controllers.V1
{
    ...
    public class JogosController : ControllerBase
    {
        ...
        public JogosController(IJogoService jogoService, JogoTagRepository jogoTagRepository, TagRepository tagRepository)
        {
        ...
        public async Task<ActionResult<IEnumerable<JogoViewModel>>> Obter([FromQuery, Range(1, int.MaxValue)] int pagina = 1, [FromQuery, Range(1, 50)] int quantidade = 6)
        {
            ...
            {
                var jogoTags = await _jogoTagRepository.ObterTagsDoJogo(jogos[i].Id);

                if (jogoTags.Count() > 0)

                {
                    for (int j = 0; j < jogoTags.Count; j++)
                    {
                        var tag = await _tagRepository.Obter(jogoTags[j].IdTag);

                        if (tag != null)
                            listaTags.Add(tag.Nome);
                    }
                }

            jogos[i].Tags = listaTags;
        ...
        public async Task<ActionResult<JogoViewModel>> Obter([FromRoute] Guid idJogo)
        {
            ...
            var listaTags = new List<string>();
            var jogoTags = await _jogoTagRepository.ObterTagsDoJogo(idJogo);

            if (jogoTags.Count > 0)
            {
                foreach (var jogoTag in jogoTags)
                {
                    var nomeTag = await _tagRepository.Obter(jogoTag.IdTag);

                    if(nomeTag!=null)
                        listaTags.Add(nomeTag.Nome);
                }
            }
            jogo.Tags = listaTags;
            return Ok(jogo);
